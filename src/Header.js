  ...
  return <>
    <header className="row px-5 pt-5">
        <nav className="header-data col-12 d-flex flex-column justify-content-center align-items-center position-relative">
            <img id='profile__img' className='rounded-circle img-fluid shadow-lg mb-3' src="./image/one.jpg" alt="Willhemz" style={{width:'8rem', height:'8rem', objectFit:'cover'}} />
            <p id='twitter' className='fs-3 text-dark fw-bold mb-0 pb-0'>willhemzDev</p>
            <p id='slack' type='button' className='fs-3 text-dark d-none' onClick={() => setSlack(!slack)}>{slack ? '*******' : 'WilliamsBalogun'}</p>
